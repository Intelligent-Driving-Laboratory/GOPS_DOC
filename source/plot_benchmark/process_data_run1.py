...
events_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),'source_data')

# Define the names of the algorithms (which are the names of the folders)
algorithms = ['DSAC2','DSAC','SAC','DDPG','PPO']
...
def get_files_path(algorithm,env):
    ...
    target_file_list = []
    target_folder = os.path.join(events_path, algorithm,env)
    if os.path.exists(target_folder):
        for folder in os.listdir(target_folder):
            if folder.startswith(algorithm):
                for files in os.listdir(os.path.join(target_folder,folder)):
                    if files.startswith('events.out'):
                        target_file_list.append(os.path.join(target_folder,folder,files))
